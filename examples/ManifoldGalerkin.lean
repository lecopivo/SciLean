...
@[simp]
theorem differential_of_cos : ∂ Math.cos = λ x dx => - dx * Math.sin x := sorry
@[simp]
theorem differential_of_sin : ∂ Math.sin = λ x dx => dx * Math.cos x := sorry
@[simp]
theorem differential_of_exp : ∂ Math.exp = λ x dx => dx * Math.exp x := sorry
...
theorem differential_of_subtype {X : Type} [Vec X] (P : X → Prop) [Vec (Subtype P)] 
  : ∂ (λ x : (Subtype P) => x.1) = λ x dx => dx.1 := sorry

@[simp] 
theorem differential_of_fst {X Y : Type} [Vec X] [Vec Y] 
  : ∂ (λ ((x,y) : X×Y) => x) = λ (x,y) (dx,dy) => dx := sorry

@[simp] 
theorem differential_of_fst' {X Y : Type} [Vec X] [Vec Y] 
  : ∂ (λ (x : X×Y) => x.fst) = λ x dx => dx.fst := sorry

-- set_option trace.Meta.Tactic.simp.discharge true in
-- set_option trace.Meta.Tactic.simp.unify true in
noncomputable
example : Impl (∂ λ p => parm'' P r p) := 
  by 
...
-- #check ∫ λ x : X => λ a b => λ da => (λ db => ⟪∂ U a da x, ∂ U b db x⟫)† 1
...
namespace AbstractManifolGalerkin
  ...
  #check λ uj ui vj => (λ vi => ⟪∂ p uj vj, ∂ p ui vi⟫)† 1

  -- rhs block for linear problems
  #check λ uj ui => (λ vi => ⟪f uj, ∂ p ui vi⟫)† 1

  #check Impl

  def manifoldGalerkinNSolution.blockwiseLinear {U} [Hilbert U] (f : X → X) (p : U → X) [IsSmooth p] (n : ℕ) (u₀ : Fin n → U) 
    (mblock : Impl (λ uj ui vj => (λ vi => ⟪∂ p uj vj, ∂ p ui vi⟫)† 1))
    (fblock : Impl (λ uj ui => (λ vi => ⟪f (p uj), ∂ p ui vi⟫)† 1)) :=
    let Uₙ := Fin n → U  -- U^n ???
  ...
  --   : Solution (λ (uₙ : ℝ → U n) => 
  --       ∀ v t, ⟪ⅆ (p n ∘ uₙ) t, ∂ (p n) (uₙ t) v⟫ = ⟪f (p n (uₙ t)), ∂ (p n) (uₙ t) v⟫)
  --     → 
