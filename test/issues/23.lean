...
-- example (f : X → Fin 5 → Fin 10 → Fin 15→ K) (hf : ∀ i j, IsDifferentiable K (f · i j))
--   : IsDifferentiable K (fun x i => f x i) := by fprop

-- example (f : X → Fin 5 → Fin 10 → Fin 15→ K) (hf : ∀ i j, IsDifferentiable K (f · i j)) (j k)
--   : IsDifferentiable K (fun x i => f x i j k) := by fprop


-- example (f : X → Fin 5 → Fin 10 → Fin 15→ K) (hf : ∀ i k, IsDifferentiable K (fun j => f · i j k)) (j k)
--   : IsDifferentiable K (fun x i => f x i j k) := by fprop
