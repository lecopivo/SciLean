...
example {X} [Hilbert X] (x : X) : ∂† (λ r : ℝ => r * x) = λ r dr' => ⟪dr', x⟫ := by diff_simp
example (r : ℝ) : ∂† (λ x : X => r * x) = λ x (dx' : X) => r * dx' := by diff_simp
example : ∂† (λ x : ℝ => x * x) = λ x dx' : ℝ => dx' * x + x * dx' := by diff_simp

example : ∂† (λ (x : X) (i : ι) => x) = λ x dx' => ∑ i, dx' i := by diff_simp
example : ∂† (λ (f : ι → X) (i : ι) => f i) = λ f df' => df' := by diff_simp
example : ∂† (λ (f : ι → X) (i : ι) => f) = λ f df' => ∑ j, df' j := by diff_simp

example (f : X → X → ℝ) [IsSmooth f] [∀ y, HasAdjDiff (λ x => f x y)] [∀ x, HasAdjDiff (λ y => f x y)]
  : ∂† (λ (x : ι → X) => ∑ i j, f (x i) (x j)) 
    =
    (λ x dx' i=>
      (∑ j, ∂† (λ y => f y (x j)) (x i) dx') + 
            ∂† (λ y => f (x j) y) (x i) dx') := 
by 
