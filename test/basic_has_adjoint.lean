...
example {n} (f : Fin n → ℝ) (c : Fin n) : HasAdjoint fun (x : Fin n → ℝ) i => f i * x (i + c) := by infer_instance
...
example {n} (f : Fin n → ℝ) : HasAdjoint (λ r i => (f i) * r) := by infer_instance done
example {n}  (g : Fin n → ℝ) (c : Fin n) 
 : HasAdjoint (fun (df : Fin n → ℝ) => sum fun (b : Fin n) => df (b + c) * g c + g (b + c) * df b) := by infer_instance done

example {n}  (g : Fin n → ℝ) (c : Fin n) : HasAdjoint fun (f : Fin n → ℝ) (b : Fin n) => f c := by infer_instance
example {n} (g : Fin n → ℝ) (c : Fin n) : HasAdjoint fun (f : Fin n → ℝ) b => f (b + c) * g c + g (b + c) * f c := by infer_instance
