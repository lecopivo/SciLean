...
@[fun_prop]
structure PlotConstant {X : Type*} [Diffeology X] {α : Sort u} (f : X → α) : Prop where
  val : ∀ n (p : Plot X n), ∃ c, ∀ m (g : ℝ^m → ℝ^n) (hg : ContDiff ℝ ⊤ g) (u : ℝ^m), f (p (g u)) = c

@[fun_prop]
theorem PlotConstant.const_rule {α : Sort u} (a : α):
  PlotConstant (fun x : X => a) := sorry

@[fun_prop]
theorem PlotConstant.comp_rule {X : Type*} [Diffeology X] (f : α → β) (g : X → α) (hg : PlotConstant g) :
  PlotConstant (fun x => f (g x)) := sorry
...
@[fun_prop]
theorem Array.size.arg_a.PlotConstant_rule [NormedAddCommGroup X] [NormedSpace ℝ X] :
    PlotConstant (fun x : Array X => x.size) := sorry

open Classical

@[fun_prop]
theorem decide.arg_p.PlotConstant_rule (f : X → Prop) (df : (x : X) → Decidable (f x)) (hf : PlotConstant f) :
    PlotConstant (fun x => @decide (f x) (df x)) := by
...
def subsetDiffeology (X : Type*) [Diffeology X] {c : X → Prop} (hc : PlotConstant c) : Diffeology {x : X // c x} where
...
theorem dite.arg_cte.DSmooth_rule
  ...
  (c : X → Prop) (hc : PlotConstant c) (hc' : PlotConstant (fun x => ¬c x))
...
example : PlotConstant (fun x : Array ℝ => decide (x.size < 10)) := by fun_prop


theorem asdf' (f : X → Prop) (df : (x : X) → Decidable (f x)) (hf : PlotConstant f) :
  PlotConstant (fun x => @decide (f x) (df x)) := by sorry
...
-- @[fun_prop]
-- theorem dite.adsf
--   (f : X → Prop) (df : (x : X) → Decidable (f x)) (hf : PlotConstant f) :
--     PlotConstant (fun x => @decide (f x) (df x)) := by
