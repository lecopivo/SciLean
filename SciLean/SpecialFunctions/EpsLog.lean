...
namespace SciLean
  ...
  argument x [Fact (ε≠0)]
    ...
    adjDiff_simp := (dx' * εpow ε x (-2)) * x by (simp[adjointDifferential]; unfold hold; simp; done)
  ...
  argument y [Fact (ε≠0)]
    ...
    hasAdjDiff := by constructor; infer_instance; simp; intro; infer_instance; done,
    adjDiff_simp := dy' * εpow ε x y * (εlog ε x) by (simp[adjointDifferential]; unfold hold; simp)
