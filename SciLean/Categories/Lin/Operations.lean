...
variable (V R D : Type) (eval) [Vec R] [SemiHilbert' V ⟨R,D,eval⟩]
instance : IsLin (SemiInner'.semiInner (S := ⟨R,D,eval⟩) : V → V → R) := sorry
instance (u : V) : IsLin (SemiInner'.semiInner (S := ⟨R,D,eval⟩) u : V → R) := sorry
