...
namespace maintests
  ...
  def test15 (f : X → X → Y) [IsSmooth f] [IsSmooth (swap f)] : IsSmooth (λ x => f x x) := by infer_instance
  def test16 (f : X → X → Y) [IsSmooth f] [IsSmooth (swap f)] : IsSmooth (λ x => f (h x) x) := by infer_instance
...
theorem test4 : IsSmooth (λ (g : X → X) x => x |> g |> f) := by infer_instance
theorem test5 : IsSmooth (λ (g : X → X) x => x |> f |> g) := by infer_instance
-- theorem test6 : IsSmooth (λ (g : X ⟿ X) x => x |> g |> g) := by infer_instance
theorem test7 : IsSmooth (λ (g : X → X) x => x |> g |> f |> f) := by infer_instance
theorem test8 : IsSmooth (λ (g : X → X) x => x |> f |> g |> f) := by infer_instance
-- theorem test9 : IsSmooth (λ (g : X ⟿ X) x => x |> g |> g |> f) := by infer_instance
theorem test10 : IsSmooth (λ (g : X → X) x => x |> f |> f |> g) := by infer_instance
