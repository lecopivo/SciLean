...
section IntegralBounded


variable
  {R} [RealScalar R]
  {X} [NormedAddCommGroup X] [AdjointSpace R X] [MeasurableSpace X] [BorelSpace X]
  {Y} [NormedAddCommGroup Y] [NormedSpace ℝ Y]

open Classical in
theorem integral_over_bounding_ball
...
syntax (name := lfunTransConvStx) "lautodiff" (config)? (discharger)?
