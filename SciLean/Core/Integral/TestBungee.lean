...
#check (∂ l, ∫' z in Icc 1 x, bungeeTension l l₂ k₁ k₂ α z)
  rewrite_by
    unfold bungeeTension
    simp (config := {zeta:=false}) only [ftrans_simp]
    unfold scalarCDeriv
    autodiff
    simp (config := {zeta:=false}) only [ftrans_simp,Set.inter_assoc]
    simp (config := {zeta:=false}) (disch:=sorry) only [frontier_levelSet,frontier_inter]
