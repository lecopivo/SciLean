...
import SciLean.Core.New.LinMap
...
class IsLinNT {X Y} [Vec X] [Vec Y] {Xs Y' : Type} [Vec Xs] [Vec Y'] 
  ...
  proof : is_linear (uncurry n f) ∧ is_smooth (uncurry n f)
...
class IsLinN {X Y} [Vec X] [Vec Y] {Xs Y' : Type} [Vec Xs] [Vec Y'] 
  (n : Nat) (f : X → Y) [Uncurry n (X → Y) Xs Y'] extends IsLinNT n f : Prop
...
instance instIsLin_is_IsSmooth {X Y} [Vec X] [Vec Y] {Xs Y' : Type} [Vec Xs] [Vec Y'] 
  (n : Nat) (f : X → Y) [Uncurry n (X → Y) Xs Y'] [inst : IsLinN n f] 
...
by
  ...
  constructor; simp[uncurry, Uncurry.uncurry]
  rw[← h]; apply F.2
...
instance {Ws W' : Type} [Vec Ws] [Vec W']
  (f : Z → W) [Uncurry n W Ws W'] [IsLinNT (n+1) f]
  (g : X → Y → Z) [IsLinNT 2 g]
  : IsLinNT (n+2) fun x y => f (g x y) := sorry

instance {Ws W' : Type} [Vec Ws] [Vec W']
  (f : Y₁ → Y₂→ W) [Uncurry n W Ws W'] [IsLinNT (n+2) f]
  (g₁ : X → Y → Z → Y₁) [IsLinNT 3 g₁]
  (g₂ : X → Y → Z → Y₂) [IsLinNT 3 g₂]
