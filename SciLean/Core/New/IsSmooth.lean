...
import SciLean.Core.New.SmoothMap
...
class IsSmoothNT {X Y} [Vec X] [Vec Y] {Xs Y' : Type} [Vec Xs] [Vec Y'] 
  (n : Nat) (f : X → Y) [Uncurry n (X → Y) Xs Y'] : Prop where
  proof : is_smooth (uncurry n f)

class IsSmoothN {X Y} [Vec X] [Vec Y] {Xs Y' : Type} [Vec Xs] [Vec Y'] 
  (n : Nat) (f : X → Y) [Uncurry n (X → Y) Xs Y'] extends IsSmoothNT n f : Prop
...
instance {Ws W' : Type} [Vec Ws] [Vec W']
  (f : Z → W) [Uncurry n W Ws W'] [IsSmoothNT (n+1) f]
  (g : X → Y → Z) [IsSmoothNT 2 g]
  : IsSmoothNT (n+2) fun x y => f (g x y) := sorry

instance {Ws W' : Type} [Vec Ws] [Vec W']
  (f : Y₁ → Y₂→ W) [Uncurry n W Ws W'] [IsSmoothNT (n+2) f]
  (g₁ : X → Y → Z → Y₁) [IsSmoothNT 3 g₁]
  (g₂ : X → Y → Z → Y₂) [IsSmoothNT 3 g₂]
  : IsSmoothNT (n+3) fun x y z => f (g₁ x y z) (g₂ x y z) := sorry
...
by
  -- have : IsSmoothNT 3 fun x y => f (g₁ x y) := sorry
  infer_instance 
...
by
  -- have : IsSmoothNT 4 fun x y z => f (g₁ x y z) (g₂ x y z) := sorry
  infer_instance
