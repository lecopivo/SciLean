...
function_properties SciLean.Index.sum {X ι : Type} [Vec X] [Index ι] (f : ι → X) : X
argument f
  IsLin := sorry_proof,
  IsSmooth := sorry_proof,
...
function_properties SciLean.Index.sum {X ι : Type} [SemiHilbert X] [Index ι] (f : ι → X) : X
argument f
  HasAdjoint := sorry_proof,
...
function_properties SciLean.LinMap.toFun {X Y ι : Type} [Index ι] [FinVec X ι] [Hilbert Y] (f : X⊸Y) (x : X) : Y
argument f
  HasAdjoint := sorry_proof,
