...
instance {X I R K : Type*}
  ...
  zero :=
    ...
    (dataArrayEquiv I K).symm ⟨x', sorry_proof⟩
  zero_spec := sorry_proof
  scal a x :=
    let N := size I
    let xptr := (dataArrayEquiv I K x).1
  ...
  dot x y :=
    let N := size I
    let xptr := (dataArrayEquiv I K x).1
    let yptr := (dataArrayEquiv I K y).1
  ...
  axpy a x y :=
    ...
    (dataArrayEquiv I K).symm ⟨y', sorry_proof⟩
