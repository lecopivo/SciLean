...
instance {X I R K : Type*}
  ...
  zero :=
    let N := nI
    let x' := BLAS.LevelOneDataExt.const N (0:K)
    fromRn X (I:=I) (K:=K) ⟨x', sorry_proof⟩
  zero_spec := sorry_proof
  scal a x :=
    let N := nI
    let xptr := (toRn I K x).1
