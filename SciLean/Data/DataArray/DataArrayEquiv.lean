...
class DataArrayEquiv (X : Type*) (I : Type*) (K : Type*)
    [IndexType I] [PlainDataType K] where
  equiv : X ≃ K^[I]
...
instance {I J X K : Type*}
    [IndexType I] [IndexType J] [PlainDataType K]
    [PlainDataType X] [DataArrayEquiv X J K] :
    DataArrayEquiv (X^[I]) (I×J) K where
  equiv := {
      toFun x :=
        let data := x.1.1
        ⟨⟨data, size I * size J, sorry_proof⟩, sorry_proof⟩
      invFun x :=
        let data := x.1.1
        ⟨⟨data, size I, sorry_proof⟩, sorry_proof⟩
...
-/
class DefaultDataArrayEquiv (X : Type*) (I K : outParam Type*) [IndexType I] [PlainDataType K]
  extends DataArrayEquiv X I K where

instance {R K I : Type*} [IndexType I] [Scalar R K] [PlainDataType K] :
    DefaultDataArrayEquiv (K^[I]) I K where

instance {K I : Type*} [PlainDataType K] [IndexType I] [IndexType J] [PlainDataType X]
...
instance {I J} [IndexType I] [IndexType J]
    {K} [PlainDataType K]
