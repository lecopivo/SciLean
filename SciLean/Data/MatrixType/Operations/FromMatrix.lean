...
-- adjoint
open Classical in
abbrev_fun_trans fromMatrix in f [InjectiveGetElem M (m×n)] : adjoint K by
...
abbrev_data_synth fromMatrix in f [InjectiveGetElem M (m×n)] : HasAdjoint K by
...
abbrev_data_synth fromMatrix in f [InjectiveGetElem M (m×n)] : HasAdjointUpdate K by
  apply hasAdjointUpdate_from_hasAdjoint
  case adjoint => data_synth
  case simp => intros; rfl

-- reverse AD
abbrev_fun_trans fromMatrix in f [InjectiveGetElem M (m×n)] : revFDeriv K by
  unfold revFDeriv
  autodiff

abbrev_data_synth fromMatrix in f [InjectiveGetElem M (m×n)] : HasRevFDeriv K by
  apply hasRevFDeriv_from_hasFDerivAt_hasAdjoint
  case deriv => intro; data_synth
  case adjoint => intros; dsimp; data_synth
  case simp => rfl

abbrev_data_synth fromMatrix in f [InjectiveGetElem M (m×n)] : HasRevFDerivUpdate K by
