...
-- adjoint
open Classical in
abbrev_fun_trans VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : adjoint K by
  equals (fun x (i : n) => x[i]) =>
    funext f
    apply AdjointSpace.ext_inner_left K
    intro z
    rw[← adjoint_ex _ (by fun_prop)]
    simp[vector_to_spec, Finset.sum_ite, Finset.filter_eq,Inner.inner,IdxType.sum_eq_finset_sum]

abbrev_data_synth VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : HasAdjoint K by
  conv => enter [3]; assign (fun (x : X) (i : n) => x[i])
  constructor
  case adjoint => intros; simp[vector_to_spec, Inner.inner,IdxType.sum_eq_finset_sum]
  case is_linear => fun_prop

abbrev_data_synth VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : HasAdjointUpdate K by
  apply hasAdjointUpdate_from_hasAdjoint
  case adjoint => data_synth
  case simp => intros; rfl

-- reverse AD
abbrev_fun_trans VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : revFDeriv K by
  unfold revFDeriv
  autodiff

abbrev_data_synth VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : HasRevFDeriv K by
  apply hasRevFDeriv_from_hasFDerivAt_hasAdjoint
  case deriv => intros; data_synth
  case adjoint => intros; dsimp; data_synth
  case simp => rfl

abbrev_data_synth VectorType.fromVec in f [IdxType.Fold' n] [InjectiveGetElem X n] : HasRevFDerivUpdate K by
