...
-- fderiv
abbrev_fun_trans axpby in alpha x beta y [Lawful X] : fderiv K by
  simp only [blas_to_module]
  fun_trans only; simp[vector_optimize]

-- forward AD
abbrev_fun_trans axpby in alpha beta x y [VectorType.Lawful X] : fwdFDeriv K by
  simp only [blas_to_module]
  autodiff; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in alpha beta x y [Lawful X] (x₀) : (HasFDerivAt (𝕜:=K) · · x₀) by
  simp only [blas_to_module]
  data_synth => enter[2]; simp[vector_optimize]

-- adoint
abbrev_data_synth axpby in alpha beta [Lawful X] : HasAdjoint K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in alpha beta [Lawful X] : HasAdjointUpdate K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in x y [Lawful X] : HasAdjoint K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in x y [Lawful X] : HasAdjointUpdate K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in alpha y [Lawful X] : HasAdjoint K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in alpha y [Lawful X] : HasAdjointUpdate K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp


-- reverse AD
abbrev_data_synth axpby in alpha x beta y [Lawful X] : HasRevFDeriv K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp

abbrev_data_synth axpby in alpha x beta y [Lawful X] : HasRevFDerivUpdate K by
  simp only [blas_to_module]
  data_synth => enter[3]; simp[vector_optimize]; to_ssa; to_ssa; lsimp
