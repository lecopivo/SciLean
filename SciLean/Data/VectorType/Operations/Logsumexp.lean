...
-- differentiable
def_fun_prop logsumexp in x with_transitive [Lawful X] : Differentiable R by
...
-- fderiv
abbrev_fun_trans logsumexp in x [Lawful X] : fderiv R by
  equals (fun x => fun dx =>L[R]
           let x' := VectorType.softmax x
           ⟪dx, x'⟫[R]) =>
    sorry_proof

abbrev_data_synth logsumexp in x [Lawful X] (x₀) : (HasFDerivAt (𝕜:=R) · · x₀) by
  apply hasFDerivAt_from_fderiv
  case deriv => conv => rhs; fun_trans; simp[vector_optimize]
  case diff => dsimp[autoParam]; fun_prop

-- forward AD
abbrev_fun_trans logsumexp in x [Lawful X] : fwdFDeriv R by
  unfold fwdFDeriv
  fun_trans; to_ssa

-- reverse AD
abbrev_fun_trans logsumexp in x [Lawful X] : revFDeriv R by
  unfold revFDeriv
  fun_trans; to_ssa

abbrev_data_synth logsumexp in x [Lawful X] : HasRevFDeriv R by
  apply hasRevFDeriv_from_hasFDerivAt_hasAdjoint
  case deriv => intros; data_synth
  case adjoint => intros; dsimp; data_synth
  case simp => conv => rhs; simp; to_ssa

abbrev_data_synth logsumexp in x [Lawful X] : HasRevFDerivUpdate R by
