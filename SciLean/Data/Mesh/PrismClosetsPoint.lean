...
def closestOnTriangle (x : ℝ^{2}) (fromRef toRef : ℝ^{2} → ℝ^{2}) : (f : Face triangle) × ℝ^{f.dim} :=
  ...
  let y₀ := y[0]
  let y₁ := y[1]
  let e₀ : ℝ^{2} := λ [i] => if i.1=0 then 1 else 0
  let e₁ : ℝ^{2} := λ [i] => if i.1=1 then 1 else 0
  let p₀ : ℝ^{2} := 0
  let p₁ : ℝ^{2} := fromRef e₀
  let p₂ : ℝ^{2} := fromRef e₁
  ...
  if y₁ ≤ 0 then
    ...
    if t ≤ 0 then
      -- point 0
      Sigma.mk triangle.point0 0
    else if 1 ≤ t then
      -- point 1
      Sigma.mk triangle.point1 0
    else 
      -- edge 0
      let t : ℝ^{1} := λ [i] => t
      Sigma.mk triangle.edge0 t
  else if y₀ ≤ 0 then
    ...
    if t ≤ 0 then
      ...
      -- point 2
      Sigma.mk triangle.point2 0
    else 
      -- edge 1
      let t : ℝ^{1} := λ [i] => t
      Sigma.mk triangle.edge1 t
...
def closestPoint (P : Prism) (x : P.Space) (fromRef toRef : P.Space → P.Space) : (f : Face P) × ℝ^{f.dim} := 
  ...
  | ⟨.cone .point, _⟩ => 
    let y : ℝ^{1} := toRef x
    if y[0] ≤ 0 then
      Sigma.mk segment.point0.anyDim 0
    else if 1 ≤ y[0] then
      Sigma.mk segment.point1.anyDim 0
    else
