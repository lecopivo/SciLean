...
import SciLean.Tactic.CSimp.Rewrite
...
-/
def Config.updateArith (c : Simp.Config) : CoreM Simp.Config := do
  if c.arith then
...
private def mkImpCongr (src : Expr) (r₁ r₂ : Result) : MetaM Result := do
  ...
  match r₁.proof?, r₂.proof? with
  | none,     none   => return { expr := e, proof? := none }
  | _,        _      => return { expr := e, proof? := (← Meta.mkImpCongr (← Result.getProof r₁) (← Result.getProof r₂)) } -- TODO specialize if bootleneck
...
where
  ...
  visitFn (e : Expr) : M Result := do
    ...
    if fNew.expr == f then
      ...
      let mut proof ← Result.getProof fNew
      for arg in args do
  ...
  tryAutoCongrTheorem? (e : Expr) : M (Option Result) := do
    ...
    for arg in args, kind in cgrThm.argKinds do
      ...
      | CongrArgKind.eq =>
        ...
        let argProof ← Result.getProof' arg argResult 
        j := j + 1
  ...
  processCongrHypothesis (h : Expr) : M Bool := do
    forallTelescopeReducing (← inferType h) fun xs hType => withNewLemmas xs do
      ...
      rhs.withApp fun m zs => do
        ...
        unless (← isDefEq h (← mkLambdaFVars xs (← Result.getProof r))) do
          throwCongrHypothesisFailed
        ...
        withSimpTheorems s do
          ...
          | some hq =>
            ...
            if rq.expr.containsFVar h.fvarId! then
              return { expr := (← mkForallFVars #[h] rq.expr), proof? := (← withDefault <| mkImpDepCongrCtx (← Result.getProof rp) hq) }
            else
              return { expr := e.updateForallE! rp.expr rq.expr, proof? := (← withDefault <| mkImpCongrCtx (← Result.getProof rp) hq) }
    else
    ...
    if (← Simp.getConfig).zeta then
      ...
      | SimpLetCase.nondep =>
        ...
        withLocalDeclD n t fun x => do
          ...
          | _,      some h => return { expr := e', proof? := some (← mkLetCongr (← Result.getProof rv) h) }
...
def main (e : Expr) (ctx : Simp.Context) (usedSimps : UsedSimps := {}) (methods : Methods := {}) : MetaM (Result × UsedSimps) := do
  let ctx := { ctx with config := (← Config.updateArith ctx.config) }
    ...
    if ctx.dischargeDepth >= ctx.config.maxDischargeDepth then
      ...
      withReader (fun ctx => { ctx with dischargeDepth := ctx.dischargeDepth + 1 }) do
        ...
        if r.expr.isConstOf ``True then
          try
            return some (← mkOfEqTrue (← Result.getProof r))
          catch _ =>
            return none
        else
          return none
...
open CSimp -- (UsedSimps)

open Lean Meta Simp

def simp (e : Expr) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
    (usedSimps : UsedSimps := {}) : MetaM (Simp.Result × UsedSimps) := do profileitM Exception "simp" (← getOptions) do
  match discharge? with
  | none   => CSimp.main e ctx usedSimps (methods := CSimp.DefaultMethods.methods)
  | some d => CSimp.main e ctx usedSimps (methods := { pre := (CSimp.preDefault · d), post := (CSimp.postDefault · d), discharge? := d })

def dsimp (e : Expr) (ctx : Simp.Context)
    (usedSimps : UsedSimps := {}) : MetaM (Expr × UsedSimps) := do profileitM Exception "dsimp" (← getOptions) do
  CSimp.dsimpMain e ctx usedSimps (methods := CSimp.DefaultMethods.methods)
...
def simpTargetCore (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
...
def simpTarget (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
...
/--
  ...
  This method assumes `mvarId` is not assigned, and we are already using `mvarId`s local context. -/
def simpStep (mvarId : MVarId) (proof : Expr) (prop : Expr) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
...
def simpLocalDecl (mvarId : MVarId) (fvarId : FVarId) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
...
def simpGoal (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
...
def simpTargetStar (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option CSimp.Discharge := none)
