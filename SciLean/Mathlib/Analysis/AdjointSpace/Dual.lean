...
we define a map `AdjointSpace.continuousLinearMapOfBilin B : E →L[𝕜] E`,
...
variable (𝕜 : Type*)
variable (E : Type*) [RCLike 𝕜] [NormedAddCommGroup E] [AdjointSpace 𝕜 E]
...
def toDualMap : E →ₗᵢ⋆[𝕜] NormedSpace.Dual 𝕜 E :=
  { innerSL 𝕜 with norm_map' := sorry_proof }
...
#align inner_product_space.innerSL_norm AdjointSpace.innerSL_norm
...
#align inner_product_space.ext_inner_left_basis AdjointSpace.ext_inner_left_basis
...
theorem ext_inner_right_basis {ι : Type*} {x y : E} (b : Basis ι 𝕜 E)
  ...
  conv_rhs => rw [← inner_conj_symm]
  exact congr_arg conj (h i)
#align inner_product_space.ext_inner_right_basis AdjointSpace.ext_inner_right_basis
  ...
  LinearIsometryEquiv.ofSurjective (toDualMap 𝕜 E)
    (by
      ...
      · rw [← Submodule.orthogonal_eq_bot_iff] at htriv
        ...
        exact h₄)
#align inner_product_space.to_dual AdjointSpace.toDual
...
theorem toDual_apply {x y : E} : toDual 𝕜 E x y = ⟪x, y⟫ :=
  rfl
#align inner_product_space.to_dual_apply AdjointSpace.toDual_apply

@[simp]
theorem toDual_symm_apply {x : E} {y : NormedSpace.Dual 𝕜 E} : ⟪(toDual 𝕜 E).symm y, x⟫ = y x := by
  rw [← toDual_apply]
  simp only [LinearIsometryEquiv.apply_symm_apply]
#align inner_product_space.to_dual_symm_apply AdjointSpace.toDual_symm_apply
...
#align inner_product_space.continuous_linear_map_of_bilin AdjointSpace.continuousLinearMapOfBilin
...
#align inner_product_space.continuous_linear_map_of_bilin_apply AdjointSpace.continuousLinearMapOfBilin_apply
...
#align inner_product_space.unique_continuous_linear_map_of_bilin AdjointSpace.unique_continuousLinearMapOfBilin
