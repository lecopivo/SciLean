...
theorem E_induction (x₀ : C 0) (r : (n : Nat) → Rand (D n)) (f : (n : ℕ) → C n → D n → (C (n+1))) :
    ...
    =
    (Nat.recOn (motive:=fun n => Rand (C n)) n (pure x₀) (fun n x => x.bind fun x' => (r n).bind fun y => pure (f n x' y))).mean := by
  induction n
  case zero => simp[mean]
  case succ n hn =>
    simp[hn,mean]
...
#eval print_mean_variance1 (foo' 0.5 [1.0,2,3,4] 4) 1000 "hihi"
