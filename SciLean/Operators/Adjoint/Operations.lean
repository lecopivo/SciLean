import SciLean.Operators.Adjoint.Atomic
...
namespace SciLean
  ...
  instance has_adjoint_smul2 (r : ℝ)
    : HasAdjoint (λ x : X => r * x) := by infer_instance
  @[simp]
  theorem adjoint_of_smul2 (r : ℝ)
    : (λ x : X => r * x)† = λ x : X => r * x := by simp
  @[simp]
  ...
  instance has_adjoint_smul1 
    {X : Type} [Hilbert X] (x : X)
    : HasAdjoint (λ r : ℝ => r * x) := by infer_instance
  theorem adjoint_of_smul1
    {X : Type} [Hilbert X] (x : X)
    : (λ r : ℝ => r * x)† = λ y : X => ⟪x, y⟫ := by simp

  -- Addition --
  --------------

  instance : HasAdjoint (λ ((x,y) : X×X) => x + y) := by infer_instance
  instance : HasAdjoint (λ ((x,y) : X×X) => x - y) := by infer_instance
  ...
  theorem adjoint_of_inner2 
    {X : Type} [Hilbert X] 
    (x : X) 
    : (λ y : X => ⟪x, y⟫)† = (λ (s : ℝ) => s * x) := by simp done
  ...
  instance has_adjoint_sum
   : HasAdjoint (sum : (ι → X) → X) := by infer_instance

  @[simp]
  theorem adjoint_of_sum
    : (sum : (ι → X) → X)† = λ x i => x := by simp
