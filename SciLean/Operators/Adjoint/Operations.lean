...
namespace Adjoint
  ...
  @[simp]
  theorem adjoint_of_inner_1' {X Y : Type} [Hilbert X] [Hilbert Y] (f : X → Y) [HasAdjoint f] (y : Y) : adjoint (λ x : X => ⟪f x, y⟫) = (λ (s : ℝ) => s * (adjoint f) y) := by funext r; autoadjoint; simp; admit
  @[simp]
