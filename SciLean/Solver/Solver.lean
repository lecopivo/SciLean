...
macro_rules
  | `($mods:declModifiers approx $id $params:bracketedBinder* := $body by $rewrites:tacticSeq) =>
    `($mods:declModifiers def $id $params:bracketedBinder* := (by ($rewrites); (try apply Approx.exact) : Approx $body))
...
macro "approx_limit " n0:term : tactic =>
 `(tactic| ((conv => enter [1]; bubble_lim; (tactic => sorry_proof)); apply (Approx.limit _ ($n0:term))))
